...
public class SmileUtils {
	 ...
	 * @return
	 */
	public static boolean addSmiles(Context context, Spannable spannable,float scale) {
		...
		for (Entry<Pattern, Integer> entry : emoticons.entrySet()) {
			...
			while (matcher.find()) {
				...
				if (set) {
					...
					ImageSpan1 s1 = new ImageSpan1(context, entry.getValue());
					s1.setmScale(scale);
					spannable.setSpan(s1, matcher.start(), matcher.end(),
	...
	public static Spannable getSmiledText(Context context, CharSequence text,float scale) {
		Spannable spannable = spannableFactory.newSpannable(text);
		addSmiles(context, spannable,scale);
		return spannable;
	}
