...
public class CallActivity extends BaseActivity {
	...
	protected void saveCallRecord(int type) {
		...
		switch (callingState) {
		case NORMAL:
			txtBody = new TextMessageBody("通话结束 " + callDruationText);
			break;
		case REFUESD:
			txtBody = new TextMessageBody("通话已挂断");
			break;
		case BEREFUESD:
			txtBody = new TextMessageBody("通话被拒绝");
			break;
		case OFFLINE:
			txtBody = new TextMessageBody("对方不在线");
			break;
		case BUSY:
			txtBody = new TextMessageBody("服务器繁忙,请稍后再试");
			break;
		case NORESPONSE:
			txtBody = new TextMessageBody("对方未接听");
			break;
		case UNANSWERED:
			txtBody = new TextMessageBody("对方未回复");
			break;
		default:
			txtBody = new TextMessageBody("通话已被取消");
			break;
		}
		// 设置扩展属性
		if (type == 0)
